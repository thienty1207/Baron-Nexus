...
- A fresh native `0.1.1` release was built in `/tmp/baron-release-0.1.1.final.akD6Ay` with Go 1.27.0, CGO disabled, Linux amd64 and Windows amd64 artifacts, installers, manifest, module SBOM, toolchain metadata, and verified `SHA256SUMS`. The release gate remains `BLOCKED` for incomplete P19/P22/P27 evidence and public publication.
- The real `/home/ty/project-test` invocation of `baron install` returned the
  expected GitHub release HTTP 404 because `v0.1.1` is not public yet; it did
  not bypass release verification. The rebuilt 0.1.1 binary was separately
  rerun through DSH, Codex, Tencent, setup, and repair, with readiness still
  limited only by this agent terminal's uncached sudo/Docker access.
