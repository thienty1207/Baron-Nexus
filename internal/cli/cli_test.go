...
func TestVersionFlagUsesBaronFormat(t *testing.T) {
	var out bytes.Buffer
	cmd := New(Options{Version: "0.1.4", Out: &out, Err: &out})
	cmd.SetArgs([]string{"--version"})
	if err := cmd.Execute(); err != nil {
		t.Fatal(err)
	}
	if got := strings.TrimSpace(out.String()); got != "baron 0.1.4" {
		t.Fatalf("version output=%q", got)
	}
}

func TestDefaultVersionIsBaron014(t *testing.T) {
	if version.Value != "0.1.4" {
		t.Fatalf("default version=%q, want 0.1.4", version.Value)
	}
}
